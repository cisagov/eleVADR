...
@pytest.mark.parametrize("src_ip, dst_ip, expected", [
    # Private to Private (Lateral)
    ("192.168.1.1", "192.168.1.10", "lateral"),
    ("10.0.0.1", "172.16.0.1", "lateral"),
    # Private to Global (Outbound)
    ("192.168.1.1", "8.8.8.8", "outbound"),
    ("fe80::1", "2001:db8::1", "outbound"), # fe80::/10 is link-local, not private, but is_private returns True for it
    # Global to Private (Inbound)
    ("8.8.8.8", "192.168.1.1", "inbound"),
    ("2001:db8::1", "fe80::1", "inbound"),
    # Global to Global (External)
    ("8.8.8.8", "4.4.4.4", "external"),
    ("2001:db8::1", "2001:db8::2", "external"),
    # Invalid IPs
    ("invalid-src", "192.168.1.1", None),
    ("192.168.1.1", "invalid-dst", None),
    # Other cases (e.g., private to global multicast)
    ("192.168.1.1", "224.0.0.1", "outbound"), # Multicast is not private, so private to multicast is outbound
    ("192.168.1.1", "ff02::1", "outbound"),
])
...
@pytest.mark.parametrize("src_ip, dst_ip, protocol_ver_id, expected_src_subnet, expected_dst_subnet", [
    # IPv4
...
def test_subnet_membership(src_ip, dst_ip, protocol_ver_id, expected_src_subnet, expected_dst_subnet):
    row = pd.Series({
...
def mock_port_risk_df():
    return pd.DataFrame({
        ...
        'description': ['Web traffic', 'Secure Web', 'File Transfer', 'Domain Name'],
        ...
        'protocol_posture': ['Conditionally Risky', 'Inherently Risky', 'Inherently Risky', 'Conditionally Risky'],
    })

@pytest.mark.parametrize("port, expected_service_name, expected_port_type, expected_risk_categories", [
    ...
    (23, "UNKNOWN_PRIV 23", PortType.UNKNOWN_PRIV.name, "Legacy Protocol, Unknown Service"),
    # Unknown port
    (12345, "UNKNOWN 12345", PortType.UNKNOWN.name, "Unknown Service"),
    # Ephemeral port
    (50000, "EPHEMERAL 50000", PortType.EPHEMERAL.name, None),
])
def test_service_processing(port, expected_service_name, expected_port_type, expected_risk_categories,
                            mock_ports_df, mock_port_risk_df):
    ...
    normalized_expected_risk = _normalize_categories(expected_risk_categories.split(', ')) if expected_risk_categories else None
    ...
    if expected_port_type == PortType.KNOWN.name and port in mock_port_risk_df['port'].values:
        ...
        assert result_row["service.information_categories"] == _normalize_categories(pr_row['information_categories'])
    elif expected_port_type == PortType.KNOWN.name and port in mock_ports_df['port'].values:
        ...
        if expected_port_type == PortType.UNKNOWN_PRIV.name:
            assert "Unassigned well-known port number" in result_row["service.description"]
...
def test_service_processing_port_risk_no_service_name_raises_error(mock_ports_df):
    ...
    with pytest.raises(ValueError, match="Port 9999 does not have a service name defined!"):
...
@pytest.mark.parametrize("mac, expected_manufacturer", [
...
def test_set_manufacturers(mac, expected_manufacturer, mock_manufacturers_df):
    row = pd.Series({"device.mac": mac, "device.manufacturer": None})
...
def sample_traffic_df():
    return pd.DataFrame({
        ...
        'dst_endpoint.ip': ['192.168.1.10', '10.0.0.10', '10.0.0.20', '192.168.1.20'],
        'service.is_ot': [False, True, False, True], # Service from 192.168.1.2 and 10.0.0.2 is OT
    })

@pytest.mark.parametrize("ipv4_ips, ipv6_ips, expected_is_ot", [
...
@pytest.mark.parametrize("device_is_ot, device_ipv4_ips, device_ipv6_ips, ot_ips, expected_is_ot_after_check", [
    (True, ["192.168.1.1"], [], {"10.0.0.10"}, True), # Already OT, should remain OT
    (False, ["192.168.1.1"], [], {"10.0.0.10"}, False), # Not OT, no communication with OT
    (False, ["192.168.1.1"], [], {"192.168.1.10"}, True), # Not OT, communicates with OT
    (False, ["10.0.0.1"], [], {"10.0.0.20"}, True), # Not OT, communicates with OT
    (False, [], [], {"10.0.0.10"}, False), # No IPs for device
    (False, None, None, {"10.0.0.10"}, False), # No IPs for device
])
def test_is_communicating_with_ot_hosts(device_is_ot, device_ipv4_ips, device_ipv6_ips, ot_ips, expected_is_ot_after_check, sample_traffic_df):
    row = pd.Series({
...
@pytest.mark.parametrize("data, column_name, expected_counts", [
    (pd.DataFrame({"col": ["A, B", "A", "C, B"]}), "col", {"A": 2, "B": 2, "C": 1}),
    (pd.DataFrame({"col": ["A"]}), "col", {"A": 1}),
    (pd.DataFrame({"col": ["A, A"]}), "col", {"A": 2}), # Should count each occurrence
    ...
    (pd.DataFrame({"col": [None, np.nan, ""]}), "col", {}), # DataFrame with only empty/null values
])
